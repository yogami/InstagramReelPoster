...
/**
 * Kie.ai music generation client that wraps the Suno API.
 */
export class KieMusicGeneratorClient implements IMusicGeneratorClient {
    ...
    private async startGeneration(request: MusicGenerationRequest): Promise<string> {
        try {
            ...
            if (!jobId) {
                throw new Error('No job ID returned from Kie.ai');
            }
