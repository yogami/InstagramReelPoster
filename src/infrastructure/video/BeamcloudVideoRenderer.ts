 ...
 */
export class BeamcloudVideoRenderer implements IVideoRenderer {
    ...
    constructor(
        ...
        if (!endpointUrl) {
            throw new Error('Beamcloud render endpoint URL is required');
        }
        this.apiKey = apiKey;
    ...
    async render(manifest: ReelManifest): Promise<RenderResult> {
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new Error(`Beamcloud render failed: ${message}`);
            }
            throw error;
        }
    }
    ...
    private extractVideoUrl(data: any): string {
        ...
        throw new Error(`Could not extract video URL from Beamcloud render response: ${JSON.stringify(data).substring(0, 300)}`);
    }
}
