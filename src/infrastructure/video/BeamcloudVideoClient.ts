 ...
 */
export class BeamcloudVideoClient implements IAnimatedVideoClient {
    ...
    constructor(
        apiKey: string,
        endpointUrl: string,
        timeout: number = 300000 // 5 minutes - video takes longer than images
    ) {
        if (!apiKey) {
            throw new Error('Beamcloud API key is required');
        }
        if (!endpointUrl) {
            throw new Error('Beamcloud video endpoint URL is required');
        }
        this.apiKey = apiKey;
    ...
    async generateAnimatedVideo(options: AnimatedVideoOptions): Promise<AnimatedVideoResult> {
        ...
        try {
            console.log(`[Beam.cloud Mochi] Generating video: "${prompt.substring(0, 60)}..."`);
            ...
            console.log(`[Beam.cloud Mochi] Video generated in ${elapsed}s`);
            ...
            if (axios.isAxiosError(error)) {
                ...
                console.error(`[Beam.cloud Mochi] Generation failed:`, error.response?.data || error.message);
                throw new Error(`Beamcloud video generation failed: ${message}`);
            }
            throw error;
        }
    }
    ...
    private extractVideoUrl(data: any): string {
        ...
        throw new Error(`Could not extract video URL from Beamcloud response: ${JSON.stringify(data).substring(0, 300)}`);
    }
}
