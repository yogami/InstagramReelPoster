...
/**
 * Beam.cloud image generation client for self-hosted FLUX1 model.
 * Calls a deployed FLUX1 endpoint on Beam.cloud infrastructure.
 */
export class BeamcloudImageClient implements IImageClient {
    ...
    constructor(
        ...
        if (!apiKey) {
            throw new Error('Beamcloud API key is required');
        }
        if (!endpointUrl) {
            throw new Error('Beamcloud endpoint URL is required');
        }
        this.apiKey = apiKey;
    ...
    async generateImage(
        ...
        try {
            console.log(`[Beam.cloud FLUX1] Generating image...`);
            ...
            console.log(`[Beam.cloud FLUX1] Image generated in ${elapsed}s`);

            return { imageUrl };
        } catch (error) {
            if (axios.isAxiosError(error)) {
                const message = error.response?.data?.error || error.response?.data?.message || error.message;
                console.error(`[Beam.cloud FLUX1] Generation failed:`, error.response?.data || error.message);
                throw new Error(`Beamcloud image generation failed: ${message}`);
            }
            throw error;
        }
    }
    ...
    /**
     * Extracts image URL/base64 from Beam.cloud response.
    ...
    private extractImageUrl(data: any): string {
        ...
        throw new Error(`Could not extract image from Beamcloud response: ${JSON.stringify(data).substring(0, 300)}`);
    }
