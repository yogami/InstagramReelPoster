...
/**
 * Pixabay Image Client
 * Searches for free stock images using the Pixabay API instead of generating them.
 * This serves as a "Free Tier" fallback for visual content.
 */
export class PixabayImageClient implements IImageClient {
        ...
        try {
            ...
            if (!hits || hits.length === 0) {
                ...
                if (query.split(' ').length > 2) {
                    console.log('[Pixabay] No results, trying broader search...');
                    ...
                    if (retryResponse.data.hits && retryResponse.data.hits.length > 0) {
                        ...
                        return {
                            ...
                            revisedPrompt: `(Pixabay Search) ${broadQuery}`
                        };
                    }
                }

                throw new Error(`No images found on Pixabay for query: ${query}`);
            }
            ...
            return {
                imageUrl: randomHit.largeImageURL || randomHit.webformatURL,
                revisedPrompt: `(Pixabay Search) ${query}`
            };
