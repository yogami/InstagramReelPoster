...
def render_video(
    ...
    try:
        print(f"[FFmpeg] Job {job_id}: Target Renderer V14 (Production Parity)")
        ...
        print(f"[FFmpeg] Executing command: {' '.join(cmd)}")
        try:
...
def build_ffmpeg_command(
    ...
    # Concat Visuals
    concat_vis_inputs = "".join(vis_norm_tags)
    filter_complex.append(f"{concat_vis_inputs}concat=n={len(visual_inputs)}:v=1:a=0[v_base]")
    v_final_tag = "v_base"
    
    # Subtitles
    if subtitles_path:
        filter_complex.append(f"[{v_final_tag}]subtitles={subtitles_path}:force_style='FontName=Arial,FontSize=24,PrimaryColour=&H00FFFFFF,BackColour=&H80000000,BorderStyle=3,Outline=1,Shadow=0,MarginV=60'[v_burned]")
        v_final_tag = "v_burned"
    ...
    if music_path:
        ...
        filter_complex.append(f'[0:a]aresample=44100,pan=stereo|c0=c0|c1=c1,volume=2.0[vo_norm]')
        a_final_tag = "vo_norm"
        
    print(f"[FFmpeg] Debug: Final Tags: V=[{v_final_tag}] A=[{a_final_tag}]")
    cmd.extend(['-filter_complex', ';'.join(filter_complex)])
    cmd.extend(['-map', f'[{v_final_tag}]', '-map', f'[{a_final_tag}]'])
