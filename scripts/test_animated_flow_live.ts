...
import { PixabayVideoClient } from '../src/infrastructure/video/PixabayVideoClient';
...
import { CloudinaryStorageClient } from '../src/infrastructure/storage/CloudinaryStorageClient'; // Optional
...
async function run() {
    ...
    // Real Components
    const pixabayClient = new PixabayVideoClient(config.pixabayApiKey || process.env.PIXABAY_API_KEY || '');
    const videoRenderer = new ShortstackVideoRenderer(config.shotstackApiKey || process.env.SHOTSTACK_API_KEY || '');
