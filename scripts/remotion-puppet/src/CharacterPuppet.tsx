...
/**
 * CharacterPuppet — Dual-mode character rendering.
 *
 * Mode 1 (FLUX Portraits): Renders photorealistic FLUX-generated images,
 * swapping between open/closed mouth based on audio amplitude.
 *
 * Mode 2 (SVG Fallback): Original V5 stick-figure SVG puppet.
 */

interface CharacterPuppetProps {
  baseX: number;
  baseY: number;
  isFlipped?: boolean;
  audioSrc: string;
...
export const CharacterPuppet: React.FC<CharacterPuppetProps> = ({
  baseX,
  baseY,
  isFlipped = false,
  audioSrc,
  ...
  const usePortraits = portraitOpen && portraitClosed;

  // ============ FLUX PORTRAIT MODE ============
  if (usePortraits) {
    const currentImage = isTalking ? portraitOpen : portraitClosed;
    return (
      <div
        style={{
          position: "absolute",
          left: baseX,
          bottom: baseY,
          transform: `scaleX(${isFlipped ? -1 : 1}) translateY(${breathY}px)`,
          width: 400,
          height: 500,
        }}
      >
