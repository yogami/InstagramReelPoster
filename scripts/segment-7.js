 ...
 * Segment 7 Producer
 * 
 * Records /manage page showing the Kanban board with 11 agents distributed across columns.
 */
...
const SEGMENT_7_TEXT = "Scaling security requires orchestration. Our Kanban dashboard offers a unified view of your entire autonomous workforce across multiple vendors.";
...
async function main() {
    ...
    const audioPath = path.join(OUTPUT_DIR, 'segment_7_audio.mp3');
    await synthesizeAudio(SEGMENT_7_TEXT, audioPath);
    ...
    // Step 3: Record
    const recordingDir = path.join(OUTPUT_DIR, 'temp_recording_7');
    ...
    // Smooth pan across the Kanban board
    await page.evaluate(() => {
        const board = document.querySelector('[data-testid="kanban-board"]');
        if (board) {
            board.scrollTo({ left: 300, behavior: 'smooth' });
        }
    });
    ...
    const outputPath = path.join(OUTPUT_DIR, 'segment_7.mp4');
