...
describe('Fish Audio Live Integration', () => {
    const apiKey = process.env.FISH_AUDIO_API_KEY;
    const voiceId = process.env.FISH_AUDIO_VOICE_ID;

    // Only run if credentials exist
    if (!apiKey || !voiceId) {
        console.warn('Skipping Fish Audio integration test: Missing FISH_AUDIO_API_KEY or FISH_AUDIO_VOICE_ID in .env');
        test.skip('Skipped due to missing credentials', () => { });
        return;
    }

    console.log(`[Integration] Initializing Fish Audio Client with Voice ID: ${voiceId}`);
    const client = new FishAudioTTSClient(apiKey, voiceId);

    test('Should successfully synthesize audio from Fish Audio API', async () => {
        const text = "This is a verification test for the Fish Audio integration.";
