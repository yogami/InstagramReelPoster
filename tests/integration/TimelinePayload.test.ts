...
describe('Timeline Payload Validation (Live)', () => {
    const apiKey = process.env.SHOTSTACK_API_KEY;
    const baseUrl = process.env.SHOTSTACK_BASE_URL;

    if (!apiKey) {
        console.warn('Skipping Timeline validation: No API Key');
        test.skip('No Credentials', () => { });
        return;
    }

    const renderer = new TimelineVideoRenderer(apiKey, baseUrl, 1000, 5); // Fast poll
    ...
    test('Should successfully SUBMIT a render job (Payload Validity Check)', async () => {
        ...
        try {
            console.log('[Test] Submitting render to Timeline...');
            ...
            // If the error is "Timeline render failed: ...", it means the payload WAS accepted, 
            // but the render engine failed (e.g. download error). This is ACCEPTABLE for this test.
            if (error.message.includes('Timeline render failed')) {
