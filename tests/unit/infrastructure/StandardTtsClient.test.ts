...
describe('StandardTtsClient', () => {
    ...
    describe('constructor', () => {
        test('should throw error if apiKey is empty', () => {
            expect(() => new StandardTtsClient('')).toThrow('Standard API key is required');
        });

        test('should create client with valid apiKey', () => {
            const client = new StandardTtsClient('sk-test-key');
            expect(client).toBeInstanceOf(StandardTtsClient);
        });
        ...
        test('should use custom voice if provided', () => {
            const client = new StandardTtsClient('sk-test-key', 'nova');
    ...
    describe('error handling', () => {
        test('should throw with Gpt error message on API failure', async () => {
            const client = new StandardTtsClient('sk-test-key');
            ...
            await expect(client.synthesize('Test text'))
                .rejects.toThrow('Standard TTS fallback failed: Invalid API key');
        });
        ...
        test('should throw with generic error message if no specific error', async () => {
            ...
            await expect(client.synthesize('Test text'))
                .rejects.toThrow('Standard TTS fallback failed: Network error');
        });
