...
describe('PixabayImageClient', () => {
    let client: PixabayImageClient;
    const apiKey = 'test_pixabay_key';

    beforeEach(() => {
        client = new PixabayImageClient(apiKey);
        jest.clearAllMocks();
    });

    it('should throw if api key is missing', () => {
        expect(() => new PixabayImageClient('')).toThrow('Pixabay API key is required');
    });
    ...
    it('should throw error if both attempts fail', async () => {
        ...
        await expect(client.generateImage('nothing found')).rejects.toThrow('No images found on Pixabay for query');
    });
});
