...
describe('KieMusicGeneratorClient', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        jest.useFakeTimers();
    });
    ...
    describe('constructor', () => {
        test('should throw error if apiKey is empty', () => {
            expect(() => new KieMusicGeneratorClient('')).toThrow('Kie.ai API key is required');
        });

        test('should create client with valid apiKey', () => {
            const client = new KieMusicGeneratorClient('test-api-key');
            expect(client).toBeInstanceOf(KieMusicGeneratorClient);
        });
        ...
        test('should use custom baseUrl if provided', () => {
            const client = new KieMusicGeneratorClient('test-api-key', 'https://custom.api.com');
    ...
    describe('generateMusic', () => {
        test('should start generation and poll for completion', async () => {
            const client = new KieMusicGeneratorClient('test-api-key', 'https://api.kie.ai/suno', 100, 5);
    ...
    describe('startGeneration', () => {
        test('should throw if no job ID returned', async () => {
            const client = new KieMusicGeneratorClient('test-api-key', 'https://api.kie.ai/suno', 100, 1);
    ...
    describe('pollForCompletion', () => {
        test('should poll multiple times before success', async () => {
            const client = new KieMusicGeneratorClient('test-api-key', 'https://api.kie.ai/suno', 100, 10);
        ...
        test('should throw on failed status', async () => {
            ...
            const client = new KieMusicGeneratorClient('test-api-key', 'https://api.kie.ai/suno', 10, 5);
        ...
        test('should throw on timeout', async () => {
            jest.useRealTimers(); // Use real timers for this test
            const client = new KieMusicGeneratorClient('test-api-key', 'https://api.kie.ai/suno', 10, 2);
