...
describe('ShortstackVideoRenderer', () => {
    ...
    describe('Constructor validation', () => {
        it('should throw error when API key is missing', () => {
            expect(() => new ShortstackVideoRenderer('')).toThrow('Shotstack API key is required');
        });

        it('should create renderer with valid API key', () => {
            const renderer = new ShortstackVideoRenderer(apiKey, baseUrl);
    ...
    describe('render() - Error Handling', () => {
        it('should throw descriptive error when render fails to start', async () => {
            ...
            await expect(renderer.render(manifest)).rejects.toThrow('Shotstack render failed to start: Invalid payload');
        });
        ...
        it('should throw error when render status is failed', async () => {
            ...
            await expect(renderer.render(manifest)).rejects.toThrow('Shotstack render failed: Invalid asset URL');
        });

        it('should timeout after max poll attempts', async () => {
            const renderer = new ShortstackVideoRenderer(apiKey, baseUrl, 10, 3); // 3 attempts at 10ms
            const manifest = createTestManifest();
        ...
        it('should throw when no render ID returned', async () => {
            ...
            await expect(renderer.render(manifest)).rejects.toThrow('No render ID returned from Shotstack');
        });
    ...
    describe('Rate limiting and retries', () => {
        it('should handle 404 during polling gracefully', async () => {
            const renderer = new ShortstackVideoRenderer(apiKey, baseUrl, 10, 5);
            const manifest = createTestManifest();
