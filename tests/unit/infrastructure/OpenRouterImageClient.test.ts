...
describe('OpenRouterImageClient', () => {
    ...
    describe('constructor', () => {
        test('should throw error if apiKey is empty', () => {
            expect(() => new OpenRouterImageClient('')).toThrow('OpenRouter API key is required');
        });

        test('should create client with valid apiKey', () => {
            const client = new OpenRouterImageClient('test-api-key');
            expect(client).toBeInstanceOf(OpenRouterImageClient);
        });

        test('should use default model if not provided', () => {
            const client = new OpenRouterImageClient('test-api-key');
            expect((client as any).model).toBe('google/gemini-2.5-flash-image');
        });

        test('should use custom model if provided', () => {
            const client = new OpenRouterImageClient('test-api-key', 'custom/model');
        ...
        test('should use custom baseUrl if provided', () => {
            const client = new OpenRouterImageClient('test-api-key', 'model', 'https://custom.api.com');
    ...
    describe('error handling', () => {
        test('should throw with error message on API failure', async () => {
            ...
            await expect(client.generateImage('Test'))
                .rejects.toThrow('OpenRouter image generation failed: Model not found');
        });
        ...
        test('should throw if no image can be extracted', async () => {
            ...
            await expect(client.generateImage('Test'))
                .rejects.toThrow('Could not extract image from OpenRouter response');
        });
