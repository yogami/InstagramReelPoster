...
import { MochiVideoClient } from '../../../src/infrastructure/video/MochiVideoClient';

jest.mock('axios');
const mockedAxios = axios as jest.Mocked<typeof axios>;

describe('MochiVideoClient', () => {
    ...
    describe('constructor', () => {
        test('should throw error if apiKey is empty', () => {
            expect(() => new MochiVideoClient('', endpointUrl)).toThrow('Remote API key is required');
        });

        test('should throw error if endpointUrl is empty', () => {
            expect(() => new MochiVideoClient(apiKey, '')).toThrow('Remote video endpoint URL is required');
        });

        test('should create client with valid parameters', () => {
            const client = new MochiVideoClient(apiKey, endpointUrl);
            expect(client).toBeInstanceOf(MochiVideoClient);
        });
    });
