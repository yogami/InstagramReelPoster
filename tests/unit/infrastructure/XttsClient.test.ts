...
import { XttsClient } from '../../../src/infrastructure/tts/XttsClient';
...
describe('XttsClient', () => {
    ...
    describe('constructor', () => {
        it('should throw if server URL is missing', () => {
            expect(() => new XttsClient('')).toThrow('XTTS server URL is required');
        });

        it('should create client with valid server URL', () => {
            const client = new XttsClient(serverUrl);
            expect(client).toBeDefined();
        });

        it('should remove trailing slash from server URL', () => {
            const client = new XttsClient('http://localhost:8020/');
            expect(client).toBeDefined();
        });
    });
