...
describe('GptLinkedInDraftService (AC4)', () => {
    const validApiKey = 'test-api-key';
    let service: GptLinkedInDraftService;

    beforeEach(() => {
        jest.clearAllMocks();
        service = new GptLinkedInDraftService(validApiKey);
    });
    ...
    describe('Constructor', () => {
        it('should create service with valid API key', () => {
            expect(service).toBeInstanceOf(GptLinkedInDraftService);
        });

        it('should reject empty API key', () => {
            expect(() => new GptLinkedInDraftService('')).toThrow('Gpt API key is required');
        });
    });
