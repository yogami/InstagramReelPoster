...
import { RemoteTranscriptionClient } from '../../../src/infrastructure/transcription/RemoteTranscriptionClient';
...
describe('RemoteTranscriptionClient', () => {
    ...
    describe('constructor', () => {
        test('should throw error if apiKey is empty', () => {
            expect(() => new RemoteTranscriptionClient('')).toThrow('Remote API key is required');
        });

        test('should create client with valid apiKey', () => {
            const client = new RemoteTranscriptionClient('test-api-key');
            expect(client).toBeInstanceOf(RemoteTranscriptionClient);
        });
        ...
        test('should use custom model if provided', () => {
            const client = new RemoteTranscriptionClient('test-api-key', 'custom/model');
    ...
    describe('error handling', () => {
        test('should throw on invalid response structure', async () => {
            ...
            await expect(client.transcribe('https://example.com/audio.mp3'))
                .rejects.toThrow('Remote returned an invalid response structure');
        });
        ...
        test('should throw on empty content', async () => {
            ...
            await expect(client.transcribe('https://example.com/audio.mp3'))
                .rejects.toThrow('Remote model returned empty transcription');
        });
        ...
        test('should throw with API error message on axios error', async () => {
            ...
            await expect(client.transcribe('https://example.com/audio.mp3'))
                .rejects.toThrow('Transcription failed via Remote: Rate limit exceeded');
        });
