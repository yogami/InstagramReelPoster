...
describe('RemoteVideoRenderer', () => {
    ...
    describe('constructor', () => {
        test('should throw error if apiKey is empty', () => {
            expect(() => new RemoteVideoRenderer('', endpointUrl)).toThrow('Remote API key is required');
        });

        test('should throw error if endpointUrl is empty', () => {
            expect(() => new RemoteVideoRenderer(apiKey, '')).toThrow('Remote render endpoint URL is required');
        });

        test('should create renderer with valid parameters', () => {
            const renderer = new RemoteVideoRenderer(apiKey, endpointUrl);
            expect(renderer).toBeInstanceOf(RemoteVideoRenderer);
        });
    });
    ...
    describe('error handling', () => {
        ...
        test('should throw with error message on API failure', async () => {
            ...
            await expect(renderer.render(testManifest))
                .rejects.toThrow('Remote render failed: FFmpeg processing failed');
        });
        ...
        test('should throw if no video URL in response', async () => {
            ...
            await expect(renderer.render(testManifest))
                .rejects.toThrow('Could not extract video URL from Remote render response');
        });
    });
});
