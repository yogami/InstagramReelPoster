...
import { BeamcloudImageClient } from '../../../src/infrastructure/images/BeamcloudImageClient';

jest.mock('axios');
const mockedAxios = axios as jest.Mocked<typeof axios>;

describe('BeamcloudImageClient', () => {
    ...
    describe('constructor', () => {
        test('should throw error if apiKey is empty', () => {
            expect(() => new BeamcloudImageClient('', endpointUrl)).toThrow('Beamcloud API key is required');
        });

        test('should throw error if endpointUrl is empty', () => {
            expect(() => new BeamcloudImageClient(apiKey, '')).toThrow('Beamcloud endpoint URL is required');
        });

        test('should create client with valid parameters', () => {
            const client = new BeamcloudImageClient(apiKey, endpointUrl);
            expect(client).toBeInstanceOf(BeamcloudImageClient);
        });
    });
    ...
    describe('error handling', () => {
        test('should throw with error message on API failure', async () => {
            ...
            await expect(client.generateImage('Test'))
                .rejects.toThrow('Beamcloud image generation failed: Model loading failed');
        });
        ...
        test('should throw if no image can be extracted', async () => {
            ...
            await expect(client.generateImage('Test'))
                .rejects.toThrow('Could not extract image from Beamcloud response');
        });
