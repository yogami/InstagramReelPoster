...
describe('ReelOrchestrator E2E - Happy Path', () => {
    ...
    beforeEach(() => {
        ...
        // Mock OpenAI Transcription (Whisper) - returns plain text
        ...
        // Mock OpenAI DALL-E (3 image calls)
        nock('https://api.openai.com')
            .post('/v1/images/generations')
            .times(3)
            .reply(200, loadFixture('dall-e-image.json'));

        // Mock OpenAI Subtitles (returns URL as text)
        nock('https://api.openai.com')
            .post('/v1/audio/transcriptions')
            .reply(200, { text: loadFixture('subtitles-url.txt') });
        ...
        const ttsClient = new FishAudioTTSClient('test-key', 'https://api.fish.audio', 'test-voice-id');
    ...
    it('should generate reel end-to-end successfully', async () => {
        ...
        expect(result.manifest?.subtitlesUrl).toBe('https://mock-storage.example.com/subtitles.srt');
